...
interface IProps {
  ...
  handleSubmit: (values: ImportParams) => void
  loading: boolean
}

export const ImportModalForm: FC<IProps> = (props): JSX.Element => {
  const { selectData, handleSubmit, loading } = props
  const { i18n } = useLingui()

  const handleFormSubmit = (values: ImportParams) => {
    handleSubmit({ amount: values.amount })
  }
  ...
  return (
    <Form onSubmit={formik.handleSubmit}>
      ...
      <ButtonWrap type="submit" disabled={loading} loading={loading}>
        <Trans id="side-link.import">Deposit</Trans>
      </ButtonWrap>
    </Form>
  )
}

...
import { useReinvestStatistic } from 'features/statistic/user/model'
import styled from 'styled-components'
import { TableTemplate } from 'shared/ui/atoms/table'
import { ReinvestStatistic } from 'features/statistic/types/user'
...
export const ReinvestTable: FC = () => {
  const $statistic = useReinvestStatistic()

  const properties = properties_info.map((property) => ({
    ...property,
    key: property.key as keyof ReinvestStatistic,
  }))

  return (
    <TableCardTemplate>
      <Title level={3}>
        <Trans id="table.reinvest.title" />
      </Title>
      <HideBlock>
        <TableTemplate<ReinvestStatistic>
          objects={$statistic}
